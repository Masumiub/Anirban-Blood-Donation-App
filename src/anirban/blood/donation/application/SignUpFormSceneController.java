...
public class SignUpFormSceneController implements Initializable {
    ...
    private void SignUpOnClick(ActionEvent event) {
        ...
        try {
            ...
            User u= new GeneralMember(userIDTextField.getText(), passwordTextField.getText(), fullNameTextField.getText(), 
                    bloodGroupComboBox.getSelectionModel().getSelectedItem(), userTypeTextField.getText(), lastBloodDonationDate.getValue().toString(),
                    contactNoTextField.getText(),secondContactNoTextField.getText(), locationTextField.getText() );
            
            oos.writeObject(u);
           
            } catch (IOException ex) {
            Logger.getLogger(SignUpFormSceneController.class.getName()).log(Level.SEVERE, null, ex);
            } finally {
                try {
